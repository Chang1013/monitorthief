...
window_size = 100
threshold = 60
slide_size = 40 #less than window_size!!! 10

def dataimport(path1, path2):

	xx = np.empty([0,window_size,3],float) ##
	...
	for f in input_csv_files:
		...
		tmp1 = np.array(data)
		x2 =np.empty([0,window_size,3],float) ##

		#data import by slide window
		k = 0
		while k <= (len(tmp1) + 1 - 2 * window_size):
			x = np.dstack(np.array(tmp1[k:k+window_size, 0:3]).T) ##
	...
	for ff in annotation_csv_files:
		...
		while k <= (len(tmp2) + 1 - 2 * window_size):
			...
			if RT > window_size * threshold / 100:   # Receiver to Transmitter
				y[int(k/slide_size),:] = np.array([0,1,0,0])
			elif TR > window_size * threshold / 100:  # Transmitter to Receiver 
				y[int(k/slide_size),:] = np.array([0,0,1,0])
			elif nobody > window_size * threshold / 100:  # Transmitter to Receiver 
				y[int(k/slide_size),:] = np.array([0,0,0,1])
			else:
				y[int(k/slide_size),:] = np.array([2,0,0,0])
			k += slide_size
...
for i, label in enumerate (["RT", "TR","nobody"]):
