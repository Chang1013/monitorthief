...
n_steps = window_size 
# Output folder
OUTPUT_FOLDER_PATTERN = "LR{0}_BATCHSIZE{1}_NHIDDEN{2}_our_data/"
...
cvscores = []
confusion_sum = [[0 for i in range(2)] for j in range(2)]

#data import
x_TR,x_RT,y_TR,y_RT = csv_import()

print("x_TR=", len(x_TR), " x_RT=", len(x_RT))
...
with tf.Session() as sess:
    for i in range(kk):
        ...
        #data separation
        wifi_x_train = np.r_[ x_TR[int(len(x_TR) / kk):], x_RT[int(len(x_RT) / kk):]]

        wifi_y_train = np.r_[y_TR[int(len(y_TR) / kk):], y_RT[int(len(y_RT) / kk):]]

        wifi_y_train = wifi_y_train[:,1:]

        wifi_x_validation = np.r_[ x_TR[:int(len(x_TR) / kk)], x_RT[:int(len(x_RT) / kk)]]

        wifi_y_validation = np.r_[ y_TR[:int(len(y_TR) / kk)], y_RT[:int(len(y_RT) / kk)]]
