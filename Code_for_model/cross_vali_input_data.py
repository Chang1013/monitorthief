...
#        yy = yy[::2,:]
        ...
        num_lines = sum(1 for l in open("input_files2/xx_100_50_" + str(i) + ".csv"))
        skip_idx = [x for x in range(1, num_lines) if x % SKIPROW !=0]

        xx = np.array(pd.read_csv("input_files2/xx_100_50_" + str(i) + ".csv", header=None, skiprows = skip_idx))
        yy = np.array(pd.read_csv("input_files2/yy_100_50_" + str(i) + ".csv", header=None, skiprows = skip_idx))
